# -*- coding: utf-8 -*-
"""
Created on Tue Mar 23 15:02:31 2021
...
df_features_path = r'C:\Users\u0117123\Box Sync\FWO\WP1\Point-cloud-extractions\processing\5_Clustering_classification\3SA\r10\Clustered_RFall_vegObj_points\df_features_clusterNr_noFruit.csv'
percentage = 0.10
#%%
### Timestamp 
t = time.localtime()
...
df_features = pd.read_csv(df_features_path)  #Import cluster points
nr_i = max(df_features['cluster_unique']) + 1 #number of cluster

#Empty dataframe
column_names = ["location", "id", "min_z", "max_z", "len_counts_dens", "min_slope_rel", "max_slope_rel"]
...
for i in range (0, nr_i):
    ...
    points = points.rename(columns={'z': 'z_orig', 'z_CHM': 'z'})
    ...
    n=len(points_sorted)
    perc = round(percentage * n) #mark and plot percentage lowest points
    points_sorted['perc']=0
    points_sorted.iloc[:perc, points_sorted.columns.get_loc('perc')] = 1
    filename_raw = "Cluster_" + str(i) + "_" + str(timestamp) #create filename for title plot
    ...
    #statitistics
    list_features = ['Intensity', 'EchoNumber', 'NrOfEchos', 'z']
    #mean
    ...
    column_names = ["location", "id","min_z", "max_z", 
    ...
    data_density_new = {'location': points['Location'].iloc[0],
                        'id': i,
...
data_density_loop.to_csv(ClusteredObjectPath + '\data_density_loop_clustered_noFruit.csv', sep=';', header=True)
